...
将val和test合并成valtest_meanstd用来放到train里面，一起求mean和std
@author: ygx
"""
...
path_train = os.path.join(base_dir,'round1_test_a_20181109.h5')
...
s1_train = np.array(fid_train['sen1'],dtype=np.float32) #(352366, 32, 32, 8)
s2_train = np.array(fid_train['sen2'],dtype=np.float32) #(352366, 32, 32, 10)
#label_train = np.array(fid_train['label'],dtype=np.int8)


s1_val = np.array(fid_val['sen1'],dtype=np.float32)
s2_val = np.array(fid_val['sen2'],dtype=np.float32)
...
shuffled_s1_trainval = s1_trainval[:,:,:,:]
shuffled_s2_trainval = s2_trainval[:,:,:,:]
...
fid_val_new = h5py.File(os.path.join(base_dir,'valtest_meanstd.h5'),'w')
...
fid_val_new['sen1']=shuffled_s1_trainval[:,:,:,:]
fid_val_new['sen2']=shuffled_s2_trainval[:,:,:,:]
